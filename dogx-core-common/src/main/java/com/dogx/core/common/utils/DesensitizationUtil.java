...
public class DesensitizationUtil {
    ...
    public String customChange(String eventFormattedMessage) {
        try {
            ...
            boolean flag = false;
            // 获取Yml配置文件内容 - Map格式
//            Map<String, Object> patternMap = LogBackPropUtils.patternMap;
...
//            }
    ...
    private String getReplaceValue(String value, String patternVales, String[] split, String originalPatternValues) {
        if (split.length >= 2 && !"".equals(patternVales)) {
            ...
            if (originalPatternValues.contains(">")) {
                ...
                // 获取标志符号前面数据
                newValue = indexOf > -1 ? value.substring(0, index) : "";
                // 获取数据的长度
    ...
    private Object getKeyIgnoreCase(String key) {
        //每次都去，当修改后需要
        allPattern = LogBackPropUtils.getAllPattern();
//        ignoreFlag = LogBackPropUtils.getIgnore();
...
//        }
            ...
            if (!"".equals(customRegex) && value.matches(customRegex)) {
                ...
                if (!"".equals(defaultRegex)) {
                    if (IDENTITY.equals(defaultRegex) && isIdentity(value)) {
                        ...
                        return position;
                    } else if (NAME.equals(defaultRegex)) {
                        return position;
    ...
    private Boolean checkOpen() {
        ...
        // 后面就不用去Yml里获取了
        if (LogBackPropUtils.getOpen() == null) {
            return false;
        } else {
            return LogBackPropUtils.getOpen();
        }
    }
